...
class BMPC(torch.nn.Module):
	...
	def _estimate_value(self, z, actions, task, horizon, regularization_coefficient=None):
		...
		for t in range(horizon):
			...
			if regularization_coefficient:
				G -= regularization_coefficient * self._estimate_uncertainty(z, actions[:, t], task)
			discount_update = self.discount[torch.tensor(task)] if self.cfg.multitask else self.discount
			discount = discount * discount_update
		if self.cfg.use_v_instead_q:
			action = None  # dummy value to serve the interface
		...
		if regularization_coefficient:
			val -= regularization_coefficient * self._estimate_uncertainty(z, action, task)
		return val
	...
	def _plan(self, obs, batch_size, t0=False, eval_mode=False, task=None, horizon=None, update_prev_mean=True, reanalyze=False):
		...
		for _ in range(self.cfg.iterations):
			...
			value = self._estimate_value(z, actions, task, horizon, self.cfg.regularization_coefficient).nan_to_num(0)
