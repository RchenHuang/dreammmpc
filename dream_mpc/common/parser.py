...
def parse_cfg(cfg: OmegaConf) -> OmegaConf:
	...
	# BMPC
	if not (cfg.algo == "bmpc" or cfg.algo == "dream_mpc_bmpc"):
		cfg.use_v_instead_q = False

	assert not (cfg.algo == "bmpc" and cfg.multitask), "BMPC does not support multi-task for now"
	assert not ((cfg.algo == "dream_mpc_bmpc" or cfg.algo == "dream_mpc_tdmpc2") and cfg.multitask), "Gradient-based MPC does not support multi-task for now"
