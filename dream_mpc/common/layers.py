...
def bmpc_tdmpc2_model_conversion(source_state_dict):
	"""Converts a TD-MPC2 checkpoint to a BMPC compatible format.
	
	By default, BMPC adds expl_log_std_min and expl_log_std_dif keys, which are not needed/used in TD-MPC2.
	This allows for loading TD-MPC2 checkpoints which do not contain the keys.
	"""
	keys_to_add = ["expl_log_std_min", "expl_log_std_dif"]
	for k in keys_to_add:
		if k not in source_state_dict:
			source_state_dict[k] = torch.empty(1)
