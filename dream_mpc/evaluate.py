...
from common import get_agent_cls
...
def evaluate(cfg: dict):
	...
	Most relevant args:
		`algo`: algorithm name, must be one of `[tdmpc2, bmpc, dream_mpc_tdmpc2, dream_mpc_bmpc]` (default: dream_mpc_bmpc)
	...
	# Evaluate
	if cfg.multitask:
		print(colored(f'Evaluating {cfg.algo} agent on {len(cfg.tasks)} tasks:', 'yellow', attrs=['bold']))
	else:
		print(colored(f'Evaluating {cfg.algo} agent on {cfg.task}:', 'yellow', attrs=['bold']))
	if cfg.save_video:
